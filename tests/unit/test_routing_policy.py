...
def test_the_admitted_set_is_a_whole_route_and_names_the_ruling_each_seat_comes_from() -> None:
    ...
    A4 (#397) re-establishes exactly that path — but as the retro's one *landing*, reviewed under
...
def test_the_remedys_just_check_clause_is_true_of_every_tool_it_scopes() -> None:
    ...
    `just check` tool and #358 a tenth, `tools/check_changelog.py`; the clause scopes those
    ten and names `tools/breaker.py` separately, so this asserts both halves against the
    ...
    scoped, _, rest = listed.partition("— the ten `just check` reaches —")
...
def test_the_shipped_policy_carries_no_route_exception_for_a_dead_retro_rule() -> None:
    ...
    journal entry under A4 (#397). With no retro routing rule left there is nothing for a retro
