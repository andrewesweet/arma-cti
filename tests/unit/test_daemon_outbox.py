...
def test_a_pushed_message_is_pending_until_it_is_acknowledged() -> None:
    outbox = Outbox()
    outbox.push({"kind": "order"})
    assert [entry.message for entry in outbox.pending()] == [{"kind": "order"}]


def test_sequence_numbers_are_issued_in_order_from_one() -> None:
    outbox = Outbox()
    assert [outbox.push({"n": n}).sequence for n in range(3)] == [1, 2, 3]
