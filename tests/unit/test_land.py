...
# The changelog every real branch folds into (#358): `[Unreleased]` standing with one
# category, so a fragment can append under it and a new category can land beside it.
UNRELEASED_SEED = """\
# Changelog
...
def repo(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    _commit(main, "CHANGELOG.md", UNRELEASED_SEED)
    _commit(
        main,
...
# --------------------------------------------------------- the changelog fold (#358)


def _fragment_commit(here: Path, name: str, body: str) -> None:
    """Commit one changelog fragment — the shape a branch records its entry in (#358)."""
    _commit(here, f"changelog.d/{name}", body)


def test_staging_folds_the_branchs_fragment_and_prints_the_post_fold_sha(
    repo: tuple[Path, Path, Path],
) -> None:
    """The SHA a review binds carries the fold, or the landing's own fold would orphan it.

    A verdict binds the commit the landing pushes; a fold deferred to the landing moves
    exactly that commit. Staging folds first and names what it produced — the ordering
    `stage`'s docstring stakes and this pins (#358).
    """
    origin, main, here = repo
    _commit(here, "feature.txt", "work\n")
    _fragment_commit(here, "358-fold.md", "### Changed\n\n- Folded entry. (#358)\n")
    tip = _tip(origin)
    ...
    assert report.code == 0
    assert report.lines[0] == "ok=staged"
    assert any(line.startswith("changelog=merged 1 fragment(s) as ") for line in report.lines)
    assert "commits=3" in report.lines
    head = _git("rev-parse", "HEAD", cwd=here).strip()
    assert f"head={head}" in report.lines
    # The fold commit is the tip, it touched only the changelog surfaces, and the
    # fragment it merged is gone from the tree — origin inherits nothing stale.
    assert (
        _git("log", "-1", "--format=%s", cwd=here).strip()
        == "chore(changelog): fold 1 fragment(s) into [Unreleased]"
    )
    touched = _git("show", "--name-only", "--format=", "HEAD", cwd=here).split()
    assert set(touched) == {"CHANGELOG.md", "changelog.d/358-fold.md"}
    assert "- Folded entry. (#358)" in (here / "CHANGELOG.md").read_text(encoding="utf-8")
    assert _git("status", "--porcelain", cwd=here).strip() == ""
    assert _tip(origin) == tip


def test_a_staged_and_reviewed_fragment_lands_folded_into_the_changelog(
    repo: tuple[Path, Path, Path],
    tmp_path: Path,
) -> None:
    """The happy path end to end: stage, review the SHA it printed, land.

    The second run's fold is a no-op — staging consumed the fragments — so the
    verdict bound at the staged SHA still names the commit that lands. Nothing
    downstream ever sees a fragment.
    """
    origin, main, here = repo
    _commit(here, "feature.txt", "work\n")
    _fragment_commit(here, "358-fold.md", "### Changed\n\n- Folded entry. (#358)\n")

    staged = land.stage(main, here)
    assert staged.code == 0
    review = _reviewed(here, tmp_path)
    gate = _Gate()
    ...
    assert report.code == 0
    assert report.lines[0] == "ok=landed"
    # The fold ran once, at staging: the landing's own fold found nothing to do and
    # said nothing, which is why the staged verdict still binds.
    assert not any(line.startswith("changelog=") for line in report.lines)
    assert "commits=3" in report.lines
    assert len(gate.calls) == 1
    assert (
        _git("log", "-1", "--format=%s", cwd=here).strip()
    ...
    landed = _git("show", f"{_tip(origin)}:CHANGELOG.md", cwd=here)
    assert "- Folded entry. (#358)" in landed
    assert "- Existing entry." in landed
    # Origin never carries a fragment: the only path under `changelog.d/` the fold
    # leaves behind is none at all.
    assert "changelog.d/" not in _git("ls-tree", "-r", "--name-only", _tip(origin), cwd=here)


def test_a_direct_landing_of_a_fragment_orphans_the_pre_fold_review_and_the_rerun_lands(
    repo: tuple[Path, Path, Path],
    tmp_path: Path,
) -> None:
    """A branch that never staged: the fold moves HEAD after the verdict bound it.

    The refusal is `no_review_dispatch` and not a silent pass over an unreviewed
    fold — the rung reads HEAD after the fold on purpose, so the commit that
    carries the merged entries is inside every verdict that clears a landing.
    The refused run leaves the fold behind; re-reviewing what it produced and
    landing again is the recovery, and it converges.
    """
    origin, main, here = repo
    _commit(here, "feature.txt", "work\n")
    _fragment_commit(here, "358-fold.md", "### Changed\n\n- Folded entry. (#358)\n")
    ...
    assert gate.calls == []
    assert _tip(origin) == tip
    # The fold ran before the refusal: fragments consumed, entries merged, and
    # the SHA a re-review binds is the one the next landing will push.
    assert (
        _git("log", "-1", "--format=%s", cwd=here).strip()
        == "chore(changelog): fold 1 fragment(s) into [Unreleased]"
    )
    assert not (here / "changelog.d" / "358-fold.md").exists()
    ...
    assert report.lines[0] == "ok=landed"
    assert len(gate.calls) == 1
    assert "- Folded entry. (#358)" in _git("show", f"{_tip(origin)}:CHANGELOG.md", cwd=here)


def test_a_malformed_fragment_refuses_before_the_gate_and_the_remote(
    repo: tuple[Path, Path, Path],
    tmp_path: Path,
) -> None:
    """`changelog_fragment_malformed`, in the landing's own vocabulary, writing nothing."""
    origin, main, here = repo
    _commit(here, "feature.txt", "work\n")
    _fragment_commit(here, "358-bad.md", "Prose before a heading.\n\n### Added\n\n- Entry.\n")
    gate = _Gate()
    tip = _tip(origin)
    changelog_before = (here / "CHANGELOG.md").read_text(encoding="utf-8")

    report = land.land(main, here, gate=gate, review=_reviewed(here, tmp_path))

    assert report.lines[0] == "refusal=changelog_fragment_malformed"
    assert gate.calls == []
    assert _tip(origin) == tip
    assert (here / "CHANGELOG.md").read_text(encoding="utf-8") == changelog_before
    assert (here / "changelog.d" / "358-bad.md").exists()


def test_a_dry_run_plans_the_fold_and_carries_a_refusal_into_blocked(
    repo: tuple[Path, Path, Path],
    tmp_path: Path,
) -> None:
    """The plan names what the fold would do, and a malformed fragment blocks it (#41's line)."""
    _origin, main, here = repo
    _commit(here, "feature.txt", "work\n")
    _fragment_commit(here, "358-fold.md", "### Changed\n\n- Folded entry. (#358)\n")
    review = _reviewed(here, tmp_path)

    planned = land.land(main, here, gate=_Gate(), dry_run=True, lane="zai", review=review)

    assert planned.code == 0
    assert "would_run=fold 1 changelog fragment(s) into [Unreleased]" in planned.lines

    _fragment_commit(here, "358-bad.md", "Prose before a heading.\n\n### Added\n\n- Entry.\n")
    blocked = land.land(
        main, here, gate=_Gate(), dry_run=True, lane="zai", review=_reviewed(here, tmp_path)
    )

    # The plan stays a plan — `ok=dry_run`, the refusal carried in the body and
    # the push and merge named as `would_not_run`, the exit code carrying the verdict.
    assert blocked.code == 1
    assert blocked.lines[0] == "ok=dry_run"
    assert any(line.startswith("changelog=changelog_fragment_malformed") for line in blocked.lines)
    assert any(line.startswith("would_not_run=git push origin HEAD:main") for line in blocked.lines)
