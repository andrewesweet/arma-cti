...
def island(objectives: int) -> manifest.MapManifest:
    ...
    """
    stems = [objective.id for objective in live().map_manifest.objectives]
    names = [
...
def test_the_squad_ceiling_falls_as_the_island_grows(
    ...
    assert observation.worst_case_bytes(map_manifest, table, 0) == empty_bytes
    assert observation.squad_ceiling(map_manifest, table) == ceiling
...
def test_an_island_can_outgrow_the_observation_before_a_squad_is_bought() -> None:
    ...
    assert observation.squad_ceiling(island(50), table) is not None
    assert observation.squad_ceiling(island(60), table) is None
...
def test_every_authored_map_fits_inside_one_callextension_return() -> None:
    ...
    for map_id, map_manifest in manifest.load_all(REPO / "addons" / "main" / "manifests").items():
        ceiling = observation.squad_ceiling(map_manifest, table)
...
def test_the_daemon_and_the_world_guard_at_the_same_number() -> None:
