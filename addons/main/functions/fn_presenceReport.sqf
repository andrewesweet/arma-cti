...
// The report loop's own running account, the counterpart to the effect pump's
// `cti_effectDrain` and there for the same reason: something outside the loop
// needs to know it has turned, and the server log is not readable from inside
// the world. `sent` counts reports that left, `replied` reports whose answer
// came back and was applied — so a wait on `replied` rising is a wait on the
// whole leg, out and back, rather than on a clock (#46). Separate from the
// heartbeat the watchdog reads: a report loop talking to a dead daemon completes
// no leg and is still alive, and only one of those two is the watchdog's
// business.
...
// runs on (#85).
["presence_report", _interval, [_turns], {
    params ["_turns"];

    // The whole report, built through the schema the daemon reads it with (#74).
    // Which five things the world reports and what each is called is one
    ...
    // transport error is (#97) — so a dead daemon counted as a completed round
    // trip and every waiter reading this counter was told the loop was healthy
    // while nothing was being reported at all.
    //
    ...
    // The daemon's view is authoritative, so the map is drawn from it rather
    // than from anything decided here.
    { [_x, _y] call cti_fnc_objectiveOwnerSet } forEach _owners;
    // Counted after the judgement has been applied, not on receipt: what a
    // waiter wants to know is that the leg completed, and the marker repaint is
