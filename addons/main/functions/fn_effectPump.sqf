...
private _pump = [_interval, _beat] spawn {
    ...
    // How many polls each still-unacknowledged sequence has been deferred on.
    // The backstop under the classification rather than a second copy of it: a
    // transient failure that is not actually clearing looks exactly like a
    // permanent one misfiled as transient, and neither should be able to retry
    // in silence for the rest of a session.
    private _deferrals = createHashMap;
    // Three minutes at the default cadence. Long enough that nothing the world
    // is genuinely waiting on — a Squad still spawning, a client still joining —
    // is called stuck, short enough that a human hears about it inside a Play
    // Session rather than after one.
