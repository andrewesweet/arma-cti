...
`just regress` and `just probe` both serialise on a machine-scoped lock at `~/.arma-cti/tier.lock`: the Arma tier is single-occupancy, and agent worktrees are siblings, so a repo-scoped lock would serialise nobody. A held lock is reported as `infra_unavailable` with the holder's metadata and is **not a result** — do not interpret it, and do not retry into it blindly; `--wait <secs>` bounds a queue. Evidence lands under `~/.arma-cti/runs/`, outside every worktree, and the durable record is the verdict plus evidence path plus SHA quoted into the issue the run gated. Full design: `docs/regression-tier.md`.

`hold` is the probe's window in seconds, default 150. Raise it when the probe's **subject** genuinely takes longer — `spike/probes/contact-decay.sqf` runs at 300 because no shorter window can contain the 120 s of knowledge decay it measures. State the reason in the probe's own header, which is authoritative — this table is not. This is not licence against the `timeout` row above: sizing the window to what is being measured is allowed, extending it until a flaky probe passes is the thing that row forbids. A probe that fails at 150 and passes at 300 without its subject having grown that long is a synchronisation bug, and the fix is in the probe. _(validated ×2 — #28: both probes were flaky and neither fix was a longer wait. Observers left in Reserve sit at the engine's default behaviour with weapons down and never acquired six men at 90 m across a full window; and a bearing off the leader's facing is a guess about line of sight that lands behind a hangar often enough to matter. #17: the harness itself was capping every probe at a hard-coded 180 s regardless of the caller's window; the fix honoured the subject-sized window rather than extending anything to pass.)_

## Failure classes

Every harness verdict carries a `class`. Read it before anything else. Untyped red = harness bug; fix the harness first. _(validated ×3 — Phase 0: `infra_unavailable` fired on a stale daemon holding the port and correctly refused to be a result. Phase 1: a deliberately emptied manifest produced `assertion_failed` in-world and the harness refused to pass rather than booting an empty world. #22: deleting the manifest to stage a failure took the daemon down first, `infra_unavailable` stopped that run being read as anything, and the guard reached properly classified `assertion_failed`.)_
...
- Land a convention together with its first applied instance; one living only in a design document is not yet a convention. Twice now: Phase 0's command table listed recipes that did not exist, and ADR-0016's probe headers sat on no probe for a day of corpus growth.
