...
- An elimination holds only in the context it was tested. Before inheriting one, check that context still applies: `skipLobby` sat on a closed issue's eliminated list, written off against a server whose mission was not initialised; against one that was, it worked first time. _(validated ×1 — #35 inherited #33's "an Assault razes an HQ in ~90 s", measured against an empty Base; against a garrisoned one it cost a run and a wrong diagnosis.)_
