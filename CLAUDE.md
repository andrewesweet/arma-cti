...
- An elimination — or any inherited measurement or rationale — holds only in the context it was tested. Before inheriting or designing on one, check that context still applies: `skipLobby` sat on a closed issue's eliminated list, written off against a server whose mission was not initialised; against one that was, it worked first time. _(validated ×3 — #35 inherited #33's "an Assault razes an HQ in ~90 s", measured against an empty Base; against a garrisoned one it cost a run and a wrong diagnosis. #26: "Squad count binds, island size barely matters" was measured while the Observation carried both rosters; #27/#28 had since made the enemy term grow with the map, and re-measuring before designing inverted the diagnosis — ADR-0030 charges the budget to the island because of it. #118: the review's reason for keeping the loop-entry guards — "a supervisor that can restart them" — had expired when #102 landed restart-refused; the agent re-derived a surviving reason rather than inheriting the expired one, the first application of this line to an inherited rationale rather than a measurement.)_
