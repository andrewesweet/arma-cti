...
| `just regress [--wait s] [--issues n,…] [--list] [name...]` | The in-world regression corpus in `spike/probes/`: fresh world per probe, one typed verdict each, worst class as the exit code. `--issues` selects probes by their `issues:` header, `--list` prints the selection and runs nothing | Yes (`--list`: no) | Before landing any issue that touches an in-world surface (`addons/`, `missions/`, `extension/`, the daemon's world-facing half, manifests) — **full corpus, no filter**. Names or `--issues` while iterating |
