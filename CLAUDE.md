...
- An elimination — or any inherited measurement — holds only in the context it was tested. Before inheriting or designing on one, check that context still applies: `skipLobby` sat on a closed issue's eliminated list, written off against a server whose mission was not initialised; against one that was, it worked first time. _(validated ×2 — #35 inherited #33's "an Assault razes an HQ in ~90 s", measured against an empty Base; against a garrisoned one it cost a run and a wrong diagnosis. #26: "Squad count binds, island size barely matters" was measured while the Observation carried both rosters; #27/#28 had since made the enemy term grow with the map, and re-measuring before designing inverted the diagnosis — ADR-0030 charges the budget to the island because of it.)_
...
- Land a convention together with its first applied instance; one living only in a design document is not yet a convention. Twice now: Phase 0's command table listed recipes that did not exist, and ADR-0016's probe headers sat on no probe for a day of corpus growth. _(validated ×2 — #23 landed the `just regress` table row, the lock, and the evidence conventions in the same commit as the recipe. #116 landed the vacuity rule's sentence in the same commit as its first instance — the client leg defaulted on, `unverified` in the verdict — after the tenth retro routed the adopted rule onto the fix by comment rather than landing it doc-first.)_
