...
- Record the verdict with `just review record`, then **fold it into the loop with `just review-loop sync --issue N --reviewed-sha <sha>`** — the route that opens the loop at round zero or records the next round with the severities taken from the record, so the seat under review cannot re-grade its own review on the way in (`open` and `round` cannot make that promise).
- **Adjudicate every finding above Low; nothing above Low may be left open** (`tools/review_loop.py`'s `stop_condition`). Low findings are recorded and never block. Four routes, and the ordinary one is `fixed`. `accepted_and_filed` — `just review-loop adjudicate --issue N --finding <id> --route accepted_and_filed --filed-issue <n> --conditional-on "<the named work outside the diff the harm depends on>"` — is available at `medium` or below **and only where the harm is conditional on work outside the diff**: "it only bites if someone later does X", with X named (`docs/agents/review-severity.md`, "How the levels are used"; `tools/review_loop.py`'s `_route_checks`). A defect that is in the diff under review takes `fixed`, whatever its severity. **`--conditional-on` takes a description, not an issue number** — any non-empty string passes, so a made-up condition writes a semantically empty adjudication that still clears the rung, and inventing one to clear a landing is the abuse the restriction exists to prevent.
...
**Review delivery is the dispatcher's transport, not yours** (#496). The reviewer bounds its report with the marker lines its brief supplies and does not call `gh`; `deliver_review` posts that one bounded section, and a posted review prints `review_delivery=posted` as a completion. Do not post the report yourself. `just dispatch-follow` prints `review_delivery_failed` and exits non-zero on an undelivered review — treat that exit as the stop, not as a completed run. Manual relay from the named log is deliberate and narrow: a host-side authentication or network failure, or a `report_unbounded` refusal once the report has been identified in the log (`tools/dispatch.py`'s `deliver_review`, whose refusal actions name each case; `docs/review-dispatch.md`, "Visibility is the target, not impossible loss", carries only the first). A `report_empty` refusal takes a fresh review, and the missing comment is never a clean one.
...
**Canonical dispatch is `just dispatch --seat S --issue N`, naming neither `--lane` nor `--profile`** (ADR-0071 ruling 2): the seat's preference list resolves the profile, and the two flags are both-or-neither everywhere. Name an explicit route — both `--lane` and `--profile` — only when a refusal forces the choice.
