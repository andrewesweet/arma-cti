...
**Review delivery is the dispatcher's transport, not yours** (#496). The reviewer bounds its report with the marker lines its brief supplies and does not call `gh`; `deliver_review` posts that one bounded section, and a posted review prints `review_delivery=posted` as a completion. Do not post the report yourself. `just dispatch-follow` prints `review_delivery_failed` and exits non-zero on an undelivered review — treat that exit as the stop, not as a completed run. Manual relay from the named log is deliberate, and its condition is that a bounded report is there to relay: `deliver_review` attaches the same relay action to every host-side post failure it names — `gh_timeout`, `gh_unrunnable` and `gh_refused` alike — and never retries or recovers one by itself. A refusal that leaves no attributable report, `report_empty` among them, takes a fresh review instead. `docs/review-dispatch.md`, "Visibility is the target, not impossible loss", states the host-failure case alone and is narrower than the code. The missing comment is never a clean one.
