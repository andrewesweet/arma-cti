...
#   5. leg B asserts freshness on five axes
set -uo pipefail
...
record "prng_streams_match" "$([[ -n "$a_prng" && "$a_prng" == "$b_prng" ]] && echo true || echo false)"
...
record "telemetry_leg_b_carries_leg_a_ids" \
    "$(grep -ac 'cycle-a-' "$OUT/telemetry-b.jsonl" 2>/dev/null)"
