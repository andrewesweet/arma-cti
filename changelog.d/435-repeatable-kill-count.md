...
- The mutation gate's kill count was not repeatable: one module measured 16/20
  ...
  under a per-test ceiling and reads the clock only at that ceiling boundary;
  every spawned subprocess also runs under one pinned `PYTHONHASHSEED`. The
  ...
  actually holds — a rate is fixed up to one kill, which is what `SLACK` now
  says it absorbs.
