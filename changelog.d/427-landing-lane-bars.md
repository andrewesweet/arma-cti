...
- **The breaker read inside `just land`'s never-alone rung can reach the network, and now says
  ...
  write alone. The call is bounded where it fires (one request, no retry, a 10 s timeout, every
  failure a typed unavailable reading), the live reader stays the default so a landing's record is
  what a dispatch would have met, and `LaneReach.quota_reader` is the seam every test now hands a
  refusing reader through — so no `just fast` run reaches a provider to decide a landing record.
