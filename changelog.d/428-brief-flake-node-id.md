...
- `just brief` now takes an open flake's test from a `tests/…::…` node ID in the issue body
  ...
  reads that ID whole: class-scoped IDs (`…py::TestClass::test_method[…]`) keep their class
  segment and bracketed parameter ids keep their spaces, the shape
  ...
  test answers to. The node-ID pattern reads the body alone; a title that happens to carry a
