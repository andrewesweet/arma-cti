...
- **The mutation gate's shell tracing no longer kills a test that shells into a `set -u` bash.**
  ...
  command, and any test module that drives `just` failed its baseline collect pass. The gate refused
  rather than merely reporting: it printed `?? could not run` and returned exit 2, so `just fast`
  was red and not merely noisy — which is how `tools/generate_seats.py` came to be measured by
  nothing at all. The default form `${BASH_SOURCE:-}` traces identically for every real script and
  yields an empty source for a `bash -c` body, which `read_traces` already drops as not one of this
