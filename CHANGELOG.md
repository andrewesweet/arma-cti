...
- **`just dispatch` refuses a half-failed brief by name (#349).** Composing a brief and
  ...
  marker, or `brief_lane_mismatch` when a route claim in the brief (a seat heading, or the
  identity sentence naming lane and profile) contradicts the dispatch's own resolved
  seat, lane and profile. The marker is one authority, shared with `tools/brief.py`, so
  the composer and the gate cannot drift. Nothing is launched on either refusal.
