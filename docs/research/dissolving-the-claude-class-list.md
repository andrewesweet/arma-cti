...
**Evidence that closes it**: a trial report at ten cycles, with each cycle's `dispatch_id` and landing SHA. (Written as `just admission trial-report`, which was never a verb of that tool; the reporting verb is `just trial report`. Superseded either way — ADR-0071 ruling 2 closed the `cti.admission.orchestration-trial/242` trial as inconclusive short of ten cycles, and this experiment's own next step below is to mint a fresh id rather than continue it.)
