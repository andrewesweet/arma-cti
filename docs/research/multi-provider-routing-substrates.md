...
That is exactly ADR-0061 Decision 7's requirement that a `quota_exhausted` wait be "computed, never guessed", available **before dispatch** and without inferring anything from error patterns. z.ai's side is documented but coarser: the GLM Coding Plan is a subscription with dual windows — credits resetting five hours after consumption and again every seven days, at 2,000/10,000 (Lite), 12,000/60,000 (Pro), 28,000/140,000 (Max) — with off-peak usage charged at 50%, peak being Mon–Fri 14:00–18:00 SGT. The original sweep found no documented machine-readable feed; issue #275's re-derivation found the first-party feed and records its live shape in section 9. — *primary source read*, https://docs.z.ai/devpack/overview.
...
- **The original z.ai negative aged out.** This sweep found no machine-readable quota state, but issue #275's re-derivation found z.ai's subsequently published usage plugin and its live quota endpoint (section 9).
...
## 9. Issue #275 follow-up: z.ai can supply quota evidence, but its 429 wall time is still unsafe

Re-derived 2026-08-07 from z.ai's current documentation, its official plugin source at commit `0446d0bb0bc537d97d3ab3664c4b8b9c4a0e1254`, and one authenticated read of the endpoint that source names. This supersedes only the z.ai quota-feed negative above.

**Candidate 1 — do not parse the 429 wall time.** z.ai documents business errors 1308 and 1310 as `Your limit will reset at {next_flush_time}`, but does not document that field's timezone, offset, or format beyond the placeholder. The same current documentation is explicit where a zone is part of a contract — peak hours are stated as Singapore Standard Time (UTC+8) — so that separate statement is not evidence that an unqualified error timestamp uses the same zone. The observed 1308 text, `2026-08-07 05:31:53`, is consistent with UTC+8 given the request time, but one consistent interpretation is not a specified contract or several-trip observation. Parsing it could reopen eight hours early, so candidate 1 fails closed. — *primary sources read*, https://docs.z.ai/api-reference/api-code and https://docs.z.ai/devpack/overview; *single incident observation, insufficient to assign a zone*.

**Candidate 2 — a first-party quota endpoint exists.** z.ai now documents its `glm-plan-usage` plugin as a way to query current GLM Coding Plan quota and usage. The official plugin calls authenticated `GET https://api.z.ai/api/monitor/usage/quota/limit`; it accepts only HTTP 200, reads `data.limits`, and maps the older `TOKENS_LIMIT` item to a five-hour percentage. The endpoint is absent from z.ai's published OpenAPI inventory, so the plugin source rather than an API-reference schema is the first-party integration contract. — *primary source read*, https://docs.z.ai/devpack/extension/usage-query-plugin, https://docs.z.ai/openapi.json, and https://github.com/zai-org/zai-coding-plugins/blob/0446d0bb0bc537d97d3ab3664c4b8b9c4a0e1254/plugins/glm-plan-usage/skills/usage-query-skill/scripts/query-usage.mjs.

One read on 2026-08-07 returned HTTP 200 and showed that the live schema has moved on from that parser: `data.limits` contained two `CREDIT_LIMIT` items. The five-hour-shaped item carried `unit=3`, `number=5`, `usage=2000`, `currentValue=122`, `remaining=1877`, `percentage=6`, and `nextResetTime=1786153957450`; the weekly-shaped item carried `unit=6`, `number=1`, `usage=10000`, `currentValue=3303`, `remaining=6696`, `percentage=33`, and `nextResetTime=1786559497998`; the response also carried `level=lite`. The arithmetic establishes `percentage` as used, not remaining (`122 / 2000` rounds to 6; `3303 / 10000` rounds to 33), and the reset values are Unix epoch milliseconds (`2026-08-08T01:52:37.450Z` and `2026-08-12T18:31:37.998Z`). No credential was recorded. — *first-party API behavior observed once; window names inferred from z.ai's documented two-window plan and the returned durations*.

The endpoint therefore supplies the evidence candidate 2 needs: a successful first-party reading names both current usage and quota remaining, while `nextResetTime` supplies the provider's own boundary when a window is exhausted. The implementation must parse the observed `CREDIT_LIMIT` shape rather than inherit the official plugin's stale `TOKENS_LIMIT` discriminator, preserve unknown `unit` codes instead of guessing names, and fail closed on a non-200 or unfamiliar shape. Poll timing would choose **when to look**; only these returned quota fields may choose **when to reopen**, so this does not invent the cooldown section 3.2 rejects.
