...
The corpus is everything under `spike/probes/` — membership is by directory, so a harness file that asserts nothing (a playtest fixture such as `spike/playtest/session-hold.sqf`, which holds a world open for a human) must live outside it, or every `just regress` pays its window for no verdict. Load-bearing since brief 0001 (2026-08-01), which is when the first non-asserting fixture was written. `spike/playtest/` now holds two: `session-hold.sqf`, which holds a fresh world open, and `mid-campaign.sqf` (#42), which plays the Campaign into a named mid-Campaign board through the port and *then* holds it open. The second asserts, but only about its own staging — it refuses to open the play window on a board it did not reach — which is the same line `spike/probe-prelude.sqf` draws for itself: an assertion about the scaffolding is not an assertion about the code under test, and neither is a verdict this tier scores. The corpus includes **bareworld** — the Phase-1 world with nothing asserted of it but what the mission itself builds. Bareworld is not padding: it carries the properties that had no Phase-1 home at all, and that #23 lists first — the addon resolving by name on a dedicated server, the seeded PRNG against the real engine, the daemon echoing a request id back through `callExtension`, the manifest's world being the world that got built, and the effect pump and presence report actually turning. Three of those lived only in `spike.Stratis`, the Phase-0 measurement mission nothing runs per issue.
...
The rest of the corpus covers what exists: Contact sampling of the enemy without his identity (`contacts`), Contact survival across engine knowledge decay (`contact-decay`), the projection reply and presence sampling telling both sides apart in one radius (`projection`), an AI-issued Order landing on real waypoints (`ai-commander`), both sides under Commanders at once (`two-commanders`), an Assault bringing an enemy Base's HQ down and a Defend garrisoning a side's own (`base-assault`), the addon parsing its own authored JSON (`json-manifest`), a Campaign actually ending — an unattended two-AI run won by Decapitation (`campaign-end`) — a Commander that finds a Base defended sending a force at it rather than a Squad (`massed-assault`), a company on held ground turning a marching Squad round and a platoon not (`recall`, ADR-0031), the leg between a real client and the Command Port (`client-port`), the Command Port's second principal — a real client leading a Squad, refilling that Squad and being refused for any other (`reinforce`, ADR-0040) — and the manifest guard refusing rather than half-building (`manifest-missing`, red by design).
...
### Optional legs, and the vacuity rule

**A probe's optional legs default on in the corpus run, and a leg that did not run surfaces in the verdict as `unverified`, never as green.** The `env:` header is where "on" is written down, so the corpus supplies the leg rather than a caller remembering to.

`human-commander` is the exemplar (#116). Its second half is the only thing it does that crosses the machine boundary — a Command built on a real client, sent through the mode=1 whitelist, judged on the server — and it was gated on `CTI_PROBE_CLIENT`, which defaulted to 0. So every corpus run of it went green having tested the server half alone, logged one line saying so, and the harness scored nothing off that line. This is the third of a family the retro of 2026-08-02 counted: #38's vacuous drive assertion, the massing table's dead computed value found on #107, and this. Three is the bar this project uses to codify.

The mechanism is one line per leg, in the probe, in a grammar `spike/run.sh` parses:
...
`run.sh` records the whole list as `legs=<leg>:<status> ...` in `results.env`, `regress.sh` copies it into `verdict.json` and prints it beside every verdict, and **any leg reported `unverified` makes the run `infra_unavailable`**. That class is deliberate rather than new: an unverified leg is the same kind of thing as a process list that could not be read — nothing was measured under conditions anyone can interpret — and this tier already refuses to read `infra_unavailable` as a result. The check runs after the `FAIL` sweep on both of `run.sh`'s verdict paths, so a probe that failed keeps the class it declared; an unverified leg is only the story when there is no other one.

`status=ran` is claimed at the bottom of a leg, after the thing being measured has answered, so no early exit can leave a verdict saying the leg happened. Where a leg genuinely cannot run — `just probe spike/probes/human-commander.sqf` on a machine with no Windows host — the answer is that same `infra_unavailable`, naming the leg. It is never a pass.

## The command surface

`just regress [--slots <n>] [--wait <secs>] [--issues <n,...>] [--list] [name...]` — no arguments runs the full corpus across three slots; names (`contacts`, `contact-decay`, matched against the `probe:` header / filename) run a subset while iterating. `--slots 1` is the serial tier and stays available and correct; see "The pool of slots" below for why the two are one code path. Depends on `build-shim build-addon` exactly as `probe` does. No bespoke environment variables: everything `just probe` currently takes from the caller's five variables is either fixed (mission, config, prefix) or read from the probe header (window).
...
`recall`'s window and its run time are further apart than any other row's, and the gap is the one this table exists to make readable: the window is the sum of seven phase deadlines sized for a bad draw — chiefly two acquisition waits of 90 s and 120 s, because #28's history is that acquisition is the thing that varies — and both greens acquired in about two seconds, so the corpus pays 110 s rather than 480. The only fixed cost in it is 45 s of watching a Squad *not* turn round, which is the negative half of the assertion and cannot be shortened by anything going well.
...
**The corpus is twenty probes as of 2026-08-02, and a full pool pass is 770 s (12 m 50 s).** Measured at `2afe6a7` — `reinforce` (#123) and `recall` (#104) landing after the seventeen-probe passes below — across three slots, twenty verdicts, all pass, peaking at 9,115 MiB of tier processes with 780 MiB still available. Two of those three minutes over the seventeen-probe 646 s are `recall` and `reinforce` arriving; the rest is scheduling noise, because a pool pass is bounded by its longest probe's tail rather than by the sum. `recall` ran 129 s in the pool against 110 s and 114 s alone, which is the bring-up contending with two other worlds.

`human-commander`'s window and measured time in that table are both from before #116 switched its client leg on. Its window went 150 → 420 with the leg — sized like `client-port`'s, on the same 240 s cold-start allowance for the client — and its run time now includes launching a client on the Windows host and waiting for the assignment sweep to see it. **47 s and 48 s**, measured 2026-08-02 at `102bade` on two consecutive green runs of `just regress human-commander client-port`, against 19 s for the server half alone. The window grew because the subject grew, which is the one reason CLAUDE.md allows. Those two passes are also #119's evidence: two client probes back to back, the second bringing its world up over the first's just-torn-down client, with no `infra_unavailable` in between.

Two probes grew again on 2026-08-02 with #24 and #19, both because their subject did and neither because a shorter window was flaky. `projection` went 150 → 300: it already planted a man inside `agia_marina`'s radius and waited for the marker to be repainted from the daemon's judgement, so #24's property — that `cti_fnc_presenceSample` tells WEST from EAST in one radius — rides on the same plant for a minute of window and no extra bring-up, where a probe of its own would have cost a world. `human-commander` kept its 420 but gained `CTI_AI_SIDE=EAST CTI_AI_SEED=1`: #19's demo is one world holding both kinds of Commander at once, and no probe in the corpus had one. Its enemy Squad is now the AI Commander's own purchase, waited for rather than bought by the probe, which is a stronger subject for the per-side isolation assertion than a roster the probe put there.
...
The Arma tier was single-occupancy until #47 and is now a pool of slots; what follows is the property the single lock established, which the pool generalises rather than replaces. The design of the pool itself is under "The pool of slots" below.

A slot is single-occupancy: one server install, one port block, and one machine shared with the human's play sessions. Concurrent agents serialise on an **`flock(2)` lock per slot at `~/.arma-cti/slots/N.lock`**. Slot 0's is the one `spike/tier-lock.sh` takes, and it wraps `just probe`, `just spike` and `just cycle-spike` — a hand run uses `~/arma3server` on 2402–2406, which *is* slot 0, so a hand run and a pool run exclude each other exactly where they would collide, and a pool that cannot get slot 0 runs in the slots it can get. The locks are machine-scoped, not repo-scoped, because agent worktrees are siblings and a lock inside any of them serialises nobody. `flock` rather than a pidfile because the kernel releases it when the holder dies, which is precisely the stale-holder failure Phase 0 met on a port (`infra_unavailable` on a daemon nobody had killed). Validated in anger on 2026-08-01, the day it shipped: a session limit killed an agent mid-run and the lock freed itself — no stale holder, no ghost `infra_unavailable`. The queue side has since run in anger too: the review-phase burn-down (2026-08-01/02) put three concurrent fix agents' regress runs through the one lock across an evening, every run serialised cleanly and every verdict attributed to its own sha. The dead run's leftovers are the other half of that event: an evidence directory with no `verdict.json` is not a result, and any server, daemon, or staged world the dead holder left is stale state to clear before the next run (ADR-0022) — which since #47 the next holder of that slot does, on acquire.

Acquisition is non-blocking by default. If every slot is held, the runner writes no evidence, touches no port, and exits `infra_unavailable` — per the failure-class table that is a stop, not a result — printing the holder metadata each holder wrote beside its lock (`N.lock.info`: pid, slot, started-at, worktree, branch, issue, and the command with its probe list), so the queued agent knows what it is waiting behind and roughly how long. The `issue` field comes from `CTI_TIER_ISSUE` and reads `unstated` when nobody set it — it is metadata for a human reading a queue, never something the command needs. `just regress --wait <secs>` bounds a blocking acquire for agents that would rather queue than yield; unbounded waiting is not offered, because an agent that would wait forever should be doing other work.
...
The guard is **ownership-blind on purpose**, and that had a cost worth naming (#119). Running the full corpus, `client-port` would pass, teardown would ask its client to stop, and the next probe's pre-flight two seconds later would see a still-exiting `arma3_x64.exe` and call it a play session — `infra_unavailable`, which is a stop, so the corpus abandoned the nine probes after it. Reproduced twice, on two shas. The tempting fix is to hand the guard the pid it launched so it can excuse its own; the reason not to is that a guard which can be talked into excusing one process is a guard that can be talked into excusing the human's, and "a process we did not start means stop" has to stay absolute. So the asymmetry is resolved on the other side: `taskkill /F` returns when the request has been made rather than when the process is gone, and teardown now waits for its own Windows processes to leave the list — `cti_windows_wait_gone`, bounded by `CTI_WINDOWS_EXIT_TIMEOUT` (90 s) — before the run releases the tier. A wait that expires is reported in the run's evidence and the next run's guard refuses, which that time is the correct refusal.

### The Windows client lock

The guard stays blind, so somebody else has to know whose client is in the list. Since #127 that is a **machine-scoped `flock(2)` at `~/.arma-cti/windows-client.lock`** (`spike/client-lock.sh`), taken around the client leg and nothing else.

The gap it closes is one the pool's own scheduling cannot: `client-port` and `human-commander` run in a serial tail with every other slot drained, which orders them against the rest of *this* run and against nothing outside it. Two agents gating from sibling worktrees each drain their own pool and then both want the one headed client on the one Windows host. While #125 was landing, two corpus attempts were stopped `infra_unavailable` by a sibling's client tripping the guard; the worse outcome, two clients joining two worlds through one engine profile, was available on the same race.

The lock's value is an **ordering**, and it is why release sits where it does: a holder does not let go until `cti_windows_wait_gone` has watched its own client leave the process list. So while a run holds the lock, no other run's client is in that list — a client there anyway is the human's, and the guard's refusal is the right one. Symmetrically, a client in the list while *somebody else holds the lock* is theirs, which is a thing to queue behind rather than a play session to refuse for. The guard is never told any of this; the caller reads the lock.

- **The tail** takes it (`spike/regress.sh`), not the whole pool, so two pools still overlap through their parallel phases and contend only for the length of the tail. A tail that cannot get it reports its probes `infra_unavailable` rather than dropping them into `not_run`, which carries no class and so would exit the corpus green having measured neither.
- **`run.sh`** takes it on its own client path, which covers `just probe` of a client probe. `CTI_CLIENT_LOCK_HELD=1` is the tail telling its children it already holds it — flock from a child on a file its parent holds waits for a lock that cannot be freed until the child returns.
- **Waiting is bounded and is queueing**, on #125's precedent: `--wait` on the pool, `CTI_CLIENT_LOCK_WAIT` on a hand run, zero by default. A refusal prints the holder's `.info` — pid, started-at, worktree, branch, issue, label — as the slot locks do.
- **Children must let go of it.** `flock` frees a lock only when the last open file description closes, and a background subshell inherits every descriptor its parent had, so every launch in `run.sh` calls `cti_client_lock_disown` before its `exec`. This is not hypothetical: the first `just unit` after the lock landed went red on a stub server that had outlived its `run.sh` and was still holding the lock against a `.info` its dead parent had deleted — a machine-wide stop with no holder to name. `tests/unit/test_client_lock.py` pins both the mechanism and the count of launches that use it.

### The pool of slots

Measured on #44, decided in **ADR-0028**, built on #47. **`just regress` runs across three slots by default.** A slot is a port block, a daemon, a server install, an engine profile and a world that agree; `spike/slots.sh` owns the geometry, the locks and the reclamation, and `spike/regress.sh` schedules onto them.

`--slots <n>` chooses how many. **`--slots 1` is the serial tier**, not a serial-looking version of the pool: slot 0 is `~/arma3server` on 2402–2406, which is the install and the port block this tier has always used, so the fast path and the known-correct path are one code path at different N. The default is 3 because three is what has been measured to fit — see the RAM row below — and changing it is a measurement rather than a preference.

| resource | per slot | the consumer that makes it real |
|---|---|---|
| game port block | `2402 + 100N` … `+4` | `-port=` on the server and the headless client; the Windows client's `-connect` port |
| daemon port | `9099 + N` | `cti-daemon --port` |
| shim daemon address | `CTI_DAEMON_ADDR=127.0.0.1:<slot daemon port>` | `extension/src/lib.rs`, once per process |
| server install | slot 0 `~/arma3server`, else `~/arma3server-slotN` | `run.sh` stages the mission PBO, `@cti` and the shim *into* it with `rm -rf` |
| engine profile | `-name=ctispikeN` / `ctihcN` | where the engine writes `logFile` and the `.rpt`; `run.sh` copies the server's `.rpt` into the evidence |
| evidence dir | `~/.arma-cti/runs/<stamp>-<probe>/` | unchanged, plus `slot` and `host` fields in `verdict.json` |
| slot lock | `~/.arma-cti/slots/N.lock` + `.lock.info` | `flock(2)`; slot 0's is the lock `just probe`, `just spike` and `just cycle-spike` take |

Every row has a named consumer because of the finding this build was designed against: **a slot boundary is only real where something reads it.** #44's first two-slot attempt had isolated ports, dirs, installs and daemons and **still merged**, because the shim resolves its daemon from `CTI_DAEMON_ADDR` once per process and `run.sh` set only `CTI_DAEMON_PORT`. One daemon received both worlds; the run not asserting on telemetry went green. A per-slot value nobody reads is decoration whose failure mode is a green run, and `tests/unit/test_pool_slots.py` asserts every value in the table both differs across slots and arrives in the run that used it.

The 100-port stride is BI's own (`topics/Arma_3_Server_Config_File.wiki`) and it is why a slot owns a block rather than a port: the engine binds the game port, +1 for the Steam query and +2 to the Steam master, derived from the game port with no config override left to pack them. CLAUDE.md's Contract grants [2400, 3000), which fits six slots clear of the human's 2302–2306; a slot index that would reach them is refused as a bug rather than accepted as a configuration.

**Allocation.** One `flock(2)` per slot, non-blocking, in index order until the pool has the N it asked for or the indices run out. Fewer slots than asked for is a smaller pool, not a failure — another agent holding slot 0 costs a slot, not a run. **No** slot free is `infra_unavailable` with every holder's metadata printed, which is the single lock's meaning N times over; `--wait <secs>` bounds a queue on the pool. `flock` rather than a pidfile for the property the whole design rests on: the kernel frees a dead holder's lock, with no reaper, no heartbeat and no pidfile.

That property has one edge the build had to handle. A worker inherits every slot's lock descriptor from the pool that forked it, and `flock` frees a lock only when the *last* descriptor closes — so a worker holding its siblings' descriptors would keep a dead slot's lock alive and make "who is still in slot 2?" unanswerable. Each worker therefore closes every slot but its own. An inherited descriptor is a hole in a bulkhead.

The same edge has a second face, found in #121: **a holder can die halfway.** Killing the top of a slot holder's process tree does *not* free the lock — the children carry the descriptor, and the kernel is right to keep the lock held, because something of that run is still alive. The pool already clears this where it owns the slot (see "The merge"), and where it does not, it now *says so*: a run that finds every slot busy prints the pids actually holding each lock beside the metadata, because the `pid=` in the metadata is the dead parent's and a human staring at a slot held by a process that no longer exists has been given the wrong name. Reported and not swept — killing a lock holder we do not own is exactly how the no-Arma tier killed the real one in #124.

**The memory pre-flight** (#125). A slot lock answers "is anyone else in slot 2?" and cannot answer "is there room for a third world?". On 2026-08-02 that gap produced the corpus's first memory red: two pools ran on the one 12 GB machine, `20260802T141637Z-pool` on slots 0–2 and `20260802T141804Z-pool` on slots 3–5, six worlds between them. Both bottomed out at ~39 MiB available, both watched a 2 s loop go silent for four minutes, and both typed it `node_crashed` — twenty minutes in, wearing a class that reads like the code's fault. Neither had done anything wrong by the locks; the second simply started into a machine that was already full. (That also settles the issue's open question about an unattributed 9.6 GiB of tier RSS: the sampler sums every `arma3server_x64` on the machine, and there were six.)

So the pool asks a second question before it takes a lock, in the host guard's shape: a reading, taken before anything is launched, fail-closed, with the refusal costing the caller nothing because nothing was launched. Unlike the host guard it has a middle answer, and it takes it — **the largest N the reading carries, said out loud**, rather than refusing a run the machine could still have run in two slots. The floor is `N × 2,500 MiB + 1,024 MiB`. Both numbers are measurement: #44 measured a heavy slot (server 1.14–1.23 GB, headless client 1.16–1.20 GB, daemon 35 MB) at ~2.43 GB, and three clean N=3 pool runs have since measured the whole tier at 7,293 / 7,349 / 7,365 MiB peak RSS — 2,431–2,455 MiB a slot, so ADR-0028's extrapolation held and 2,500 is it rounded up. The headroom is what those runs had left at their tightest (1,465 / 1,528 / 1,871 MiB available, and 1,014 MiB on the first pool pass at `191ee1f`), so a run that would have been one of them is not refused and the run that reached 39 MiB would have been. The reading is logged whether it refuses or not, because "how close was that?" should be answerable from a green run's own log. **`--wait` queues on it**, for the same reason it queues on the locks — a full machine is somebody else's run as surely as a held lock is, and it clears when that run ends. Met the day the pre-flight shipped: a `--wait 1800` was refused in two seconds because a sibling agent's three worlds were up, which is the wrong half of fail-closed. And one race the pre-flight does not close, said plainly so nobody reads it as if it did: two pools that start within ~90 s of each other — which is exactly what the two runs above did — both take their reading before either has loaded the machine, and both correctly say yes; a bring-up guard cannot see a run that has not allocated yet. The between-probes re-check is what catches that case, by stopping both pools taking new work once the collapse is visible, and a cross-pool reservation is deliberately unbuilt; ADR-0028's N question is settled — the human confirmed N=3 as the default on #125, 2026-08-02 — so building one is now a question of somebody needing it, not of a pending decision.

**And it is re-asked between probes**, not only at bring-up. The decision is recorded here because it is a judgement rather than an obligation: a bring-up reading cannot see somebody else arriving at minute eight — another pool, a build, a browser — and the re-check costs one `awk` over `/proc/meminfo` against a world that takes minutes, so it is close to free. The floor it uses is a different one (512 MiB, half the lowest healthy trough on record) because the question is different: N−1 worlds are already up and counted, and what is being asked is only whether any margin is left at all. Tripping it stops the pool taking *new* work and does not interrupt the work in flight — the bulkhead rule below, for the reason below — and the pool carries `infra_unavailable` itself, because the whole point is that no probe launched to carry it.

**Reclamation, on acquire.** ADR-0022's rule scoped per slot rather than per run, and built first rather than last (#58): lock release frees the lock and never the dead holder's processes. Before it launches anything, a slot's new holder kills whatever is bound to that slot's ports and whatever is running a binary out of that slot's install — read off `/proc/<pid>/exe`, because the engine is launched as `./arma3server_x64` from inside the install and its argv names the install nowhere. A command-line match would have found nothing and reported a squatted slot as clean. `~/.arma-cti/slots/N.last` names the evidence directory the slot was last writing, and a directory with no `verdict.json` means the previous holder was interrupted; the runner says so before it launches.

**The install farm.** `run.sh` stages *into* the install, so two slots cannot share one. Slot 0 is the master; every other slot is a `cp -al` hard-link farm of it, which costs no disk and about a fiftieth of a second, with `mpmissions/`, `@cti/` and `cti_shim_x64.so` broken back out — `install -m 0755` truncates *through* a hard link, so a farm that kept the shim linked would have one slot's shim overwrite the master's and every sibling's. The farm is rebuilt when the master's binary is a different inode from the clone's, which is what a Steam update leaves behind: a stale farm would run last month's engine and report it as this month's.

**Scheduling** is longest-job-first on the probe's own declared `window:`. A window is a deadline rather than a measurement, so it is an estimate — but it is the estimate that already lives in the probe, and a table of measured times inside the runner would be a second place to forget. What LJF has to get right is the tail: `campaign-end`'s ~390 s of a ~1,550 s corpus makes it the bound, so its 750 s window puts it first and no slot idles behind it. Claims are made with `mkdir`, which is atomic, so assignment is dynamic: a worker that finishes early takes the next job rather than idling beside a queue somebody else owns.

**The Windows-host probes are a serial tail, not part of that schedule.** `client-port` and `human-commander` each drive the headed client on the Windows host, of which there is one, and the guard that protects the human's play session is ownership-blind on purpose (#119) — so a second slot starting a probe while one slot's client is up would read that client as a play session and stop the corpus. They run one at a time on the lowest slot held, with the rest of the pool drained. Last rather than first, so a human who sits down to play at minute eight costs us the tail rather than the pass. The host guard itself stays pool-wide and is asked once, before any lock is taken: what it protects is a person, not a port block.

**Bulkheads and the two things that do stop the pool.** A probe's red is a verdict and its siblings carry on — report everything, filter in a separate pass. Two exceptions stop the pool taking *new* work without interrupting anything in flight, because killing a running world would manufacture the non-result being avoided. `infra_unavailable` is one: it is not a result, and a pool that keeps launching worlds past one produces more non-results N at a time. A second unexpected `node_crashed` is the other — a world failing systemically is hammered N times as hard by a pool as by a serial run, which is #58's reading of #72; the effect pump's breaker is still #72's. The count is of classes *after* `expect:` inversion, so the corpus's two deliberate `node_crashed` probes never trip it.
...
Two full green passes of all seventeen probes, back to back on 2026-08-02, at `191ee1f` plus this change. The pool pass is `~/.arma-cti/runs/20260802T084522Z-pool`; the serial one, run immediately afterwards on the same machine for the comparison, is `…20260802T090708Z-pool`.

| | serial (`--slots 1`) | pool (`--slots 3`) |
|---|---|---|
| wall | 1,599 s (26 m 39 s) | **646 s (10 m 46 s)** |
| verdicts | 17 pass | 17 pass, class for class identical |
| peak tier RSS | 2,520 MiB | **7,338 MiB** |
| least memory available | 5,302 MiB | **1,014 MiB** |

**60% off the wall, a 2.48× speed-up**, against ADR-0028's projected ~9 minutes. The projection missed by 100 s for two reasons it named itself: the Windows-host tail is ~125 s the pool cannot divide, and `campaign-end` at 388 s is nearly two thirds of the parallel phase on its own. The bound ADR-0028 predicted is the bound that bit.

**The RAM number is the one to read before anyone adds a fourth slot.** ADR-0028 extrapolated 7.3 GB for three slots from a measured N=2, and flagged that its own recommendation stood or fell on that arithmetic. Measured: **7,338 MiB of tier processes**, the arithmetic to within 1%. What the arithmetic did not carry is the headroom. The VM has 11.7 GiB and the pass left **1,014 MiB available at its tightest** — thinner than the ~1.3 GB ADR-0028's "8.6 GB available at rest" implied, because the machine carries more at rest now than it did then. Three slots fit: every probe passed its own assertions, nothing timed out, and the pass was 17 for 17. A fourth would not, and ADR-0028 already said a fourth buys nothing anyway, because past N=4 `campaign-end` is the whole schedule. **N=3 is therefore the ceiling on this machine, not a default with room above it**, and a heavier probe than `campaign-end` is a reason to re-measure rather than to assume.

Per-probe, the pool cost each probe between −1 s and +12 s against its serial time — inside the probes' own variance, which is the criterion ADR-0028's overturning row sets. Nothing passed alone and failed in a pool. There is exactly one probe whose *wall* moved materially:

| probe | serial | pool |
|---|---|---|
| `contacts` | 23 s | **61 s** |
| `two-commanders` | 211 s | 222 s |
| `campaign-end` | 376 s | 388 s |
| everything else | — | within ±4 s |

`contacts` waits for two leaders to know a man in common, read off `targetsQuery` — engine work whose rate depends on how much of the machine the engine has. Under two sibling worlds it takes 38 s longer and still lands well inside its 240 s window. That is a probe whose subject is contention-sensitive rather than a probe that got flaky, and the distinction matters for the same reason as everywhere else in this document: the window stays sized to the subject's worst case, and it was not touched.

#### The two failure cases, proved in world

**The bulkhead.** Four of the corpus's probes are red by design and genuinely fail inside their slot before `expect:` inverts them. In the pool pass, slot 2 crashed a node twice (`loop-watch`, `daemon-restart`) and slot 1 failed a manifest guard and a schema guard, while slot 0 ran `projection` and `bareworld` to green throughout. A red in one slot is a verdict, not a stop for its siblings — and the breaker did not fire on those two `node_crashed`, because it counts classes after inversion.

**A slot dying mid-run.** Proved by killing a worker with `kill -9` while its world was up (`~/.arma-cti/runs/20260802T093457Z-pool`, five probes at N=3, slot 2's worker killed 27 s into `casualties`). The run reported:

```
[regress] slot 2 claimed casualties and wrote no verdict — the worker died mid-probe; not a result (ADR-0022)
[slots] slot 2: previous holder was interrupted — …093524Z-casualties has no verdict.json, so it is not a result
[slots] slot 2: clearing a previous holder's leftovers: pids 2467184
[regress] casualties           infra_unavailable     0s  slot 2
[regress] worst class: infra_unavailable (exit 5)
```

The other four probes kept their slots and their verdicts, the orphaned server was killed rather than left squatting the port block, and all three slot locks were free immediately afterwards. `casualties` is `infra_unavailable` and not a red: nothing about the probe was measured.
...
Every run writes `~/.arma-cti/runs/<UTC-timestamp>-<probe>/`: `verdict.json`, `results.env`, the server stdout log, the server's own `.rpt` (per slot since #47, which is what makes "the newest rpt" this run's rather than whichever slot wrote last), daemon log, daemon telemetry JSONL, the extracted `CTI|` lines, and the staged `harness.sqf` (the probe *as appended*, so the evidence carries the exact source that ran even if `spike/probes/` has since moved on). `verdict.json` names the `slot` and the `host`. Machine-scoped for the same reason the lock is: worktrees are deleted after landing, and evidence that dies with its worktree is the #27 reconstruction-by-luck failure again.

A pool run also writes `~/.arma-cti/runs/<UTC-timestamp>-pool/`: the claim directories the workers arbitrated with, `ram.tsv` (memory in use, memory available and tier RSS every three seconds), and `pool.json` — the merged verdict set with the slots, the wall and the measured peak.

Retention: failures are kept until the issue that consumed them closes; passes are pruned by the runner at startup to the last three per probe, and pool directories to the last five. The durable record is not the directory but the issue: a run that gates an issue is quoted into that issue — verdict line, class, evidence path, git SHA — before the issue closes. A later session re-reading why a run passed starts at the issue comment and follows the path; if the directory has been pruned, the comment still carries the verdict and the SHA to re-run it at.
