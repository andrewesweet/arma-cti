...
- **Write a GitHub body**: compose it in a file, then pass `--body-file <path>` to
  `gh issue comment`, `gh issue create`, `gh pr comment`, or `gh pr create`. Never
  put body text in an inline `--body` string: Bash executes backticks in that
  string as command substitution before `gh` runs. This applies to the
  orchestrator's own posts as well as dispatched agents' posts.
- **Create an issue**: `gh issue create --title "..." --body-file <path>`. Use a
  quoted heredoc redirected to the file for multi-line bodies.
...
- **Close**: `gh issue close <number>` after posting any required body as a file-backed comment.

Infer the repo from `git remote -v` — `gh` does this automatically when run inside a clone.

## Closing an issue that carries acceptance criteria

Close it in the same session that lands its work. After terminal review, write one complete criterion audit to a UTF-8 file outside the worktree: address every acceptance criterion, give evidence for each criterion met, and for any criterion not met as written point to the recorded decision (ADR or issue) that superseded it — a prose note is not a supersession, and commit titles are not evidence. Pass that file to `just land --audit-file FILE`. If you inherit an issue whose work landed in an earlier session, audit every criterion against the tree before closing: that audit on #12 found an architectural pivot (callback-push to poll-and-ack) shipped but decided nowhere, and stopped it closing as "done".

Since #499, the close depends on provenance rather than comment content. On the completed landing path, `just land` posts the supplied file as one comment, appends the landed SHA and its explicit limits, records GitHub's successful posting call, and only then closes. It never scans existing comments, so a review quoting `just check`, `just unit` and `just mutation`, a comment saying those records are absent, and an audit split across several earlier comments cannot satisfy it. The documented split-audit false negative is resolved at the author interface: the whole closing audit belongs in the one file the rung posts as one comment. The implementer's earlier gate report remains on the thread for the reviewer; it is not the closing audit.

`audit_recorded=yes` verifies only that the rung's posting call succeeded. It does not verify the supplied body's completeness, accuracy or quality; review and human judgement own those questions. `audit_recorded=no` means the rung received no successful posting receipt, so the issue stays open; a timed-out post may nevertheless have reached GitHub, and the output must not be read as proof that no comment exists. If the same output says `audit_recorded=yes` and `issue_closed=no`, the audit record exists and only the separate close call failed; a hand close is then recovery, not a substitute audit. The work is already on `origin/main` on either post-landing failure, so neither changes the successful landing exit. A missing, unreadable or non-UTF-8 `--audit-file` instead refuses before any repository step.
...
A design question that gates implementation travels as its own issue and closes with **no code**: an ADR (with any CONTEXT.md term changes in the same commit) plus implementation issues in dependency order, each carrying acceptance criteria precise enough to implement without a clarifying question. The closing comment names the decision, the issues it spawned, and their ordering rationale. Exemplar: #31 → ADR-0020 + #32–#35; three implementing agents ran concurrently off those criteria and none needed to ask anything. Second: #169 → ADR-0052 + #188/#189, clean end to end in one evening, and it locates the convention's slack — the ruling comment explicitly left two names to the implementing ADR, and the agent corrected the working `commander_down` to `caller_dead` on ruling 5's own logic while finding the check's ordering load-bearing (post-resolution would type a dead leader `wrong_side`). Names and placement belong to the implementer, rulings to the session; a "correction" that touches a ruling goes back to the human instead. Raise one when a build ticket flags a decision as "not the scorer's to route around" — the flag is the trigger, and the decision ticket is what keeps the build ticket honest about its scope.
