...
not a second source of truth. In-flight, abandoned and stopped work is deliberately
outside it, so a dispatch record arriving before a landing cannot make the committed
file stale.
...
`just observatory` writes this projection from live sources. The orchestrator owns
regeneration after each landing, before the next landing's gate. `just
check-observatory` performs a fresh rebuild in a temporary store and compares the
generated bytes with the committed file, so a hand edit or an omitted landed row is
a refusal. In an implementer sandbox that cannot read the live sources, a red
`summary_mismatch` means the projection awaits orchestrator regeneration; it does
not by itself mean the projection code is broken. The check intentionally does not
compare a live dispatch population that has not landed yet.
