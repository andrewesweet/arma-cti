...
| `landing_source` | + `landing_source_reason` | `journal` where the SHA came from `cti.relation.produced`, `git_fallback` where it came from the bounded referencing-commit derivation; null means no source could recover a commit |
...
`just observatory` writes this projection from live sources. `just observatory backfill`
...
rebuild makes the coverage numerator visible in
`landing_coverage ... journal_coverage=<journalled>/<rows>`; `git_fallback` remains an
explicit source value until a later backfill proves the row.

`just check-observatory` performs a fresh rebuild in a temporary store and compares the generated bytes with
the committed file. A clean `summary_mismatch` is reported as stale and never writes
