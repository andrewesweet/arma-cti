...
not by itself mean the projection code is broken. The check intentionally ignores
dispatches on issues not yet landed, but does compare later dispatches naming landed
issues because those mutate an existing row.
