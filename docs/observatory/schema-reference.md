...
the external cache. A hand edit, an unreadable source, or any other seat remains a
refusal. The check intentionally ignores dispatches on issues not yet landed, but
does compare later dispatches naming landed issues because those mutate an existing
row.
