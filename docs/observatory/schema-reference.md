...
facts as `counted cost=... meter=...`, `unrecorded cost=... reason=...`, and
`none not_involved`; it has no cross-lane total. Every other nullable summary field
uses its `*_reason` sibling under the store's null law.

`just observatory` writes this projection. `just check-observatory` performs a fresh
rebuild in a temporary store and compares the generated bytes with the committed
file, so a hand edit or an omitted landed row is a refusal. The check intentionally
does not compare a live dispatch population that has not landed yet.
